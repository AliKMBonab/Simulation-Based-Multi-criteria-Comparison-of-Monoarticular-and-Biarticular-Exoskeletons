...
The last stage of this study stepped forward toward providing more realistic results from the simulations. This phase of the study revealed that the performance of devices was profoundly affected by the inertial properties of devices, which was more severe for the monoarticular device due to its kinematic design. Analysis of optimal devices under devices' inertial properties effect using developed model, and modified augmentation factor showed that most of the optimal biarticular exoskeletons were able to preserve their optimal performance under their mass and inertia effect, which ease designing an optimal device. Unlike the biarticular exoskeleton, the developed model, along with the modified augmentation factor, showed that the optimality monoarticular devices profoundly altered by reflecting the effect of their inertial properties. To improve the performance of the monoarticular exoskeleton, we suggested two different monoarticular design rather than its conventional design that can mitigate the effect of device inertial properties. Finally, the performance of the devices was analyzed under regeneration, revealing the promising effect of regeneration on the optimality of devices.\\
...
\end{figure*}
The monoarticular exoskeleton can be modeled as a two-link serial manipulator as shown in figure \ref{Fig_Exos_Kinematics_Model}\subref{Fig_Monoarticular_Exo_Mechanism} where each joint was assisted by the directly joint actuator. The kinematics modeling of the monoarticular and biarticular exoskeletons in both configuration and motion level has been represented in \nameref{S1_Appendix}.\\
As it can be interpreted from the kinematics of exoskeletons represented in \nameref{S1_Appendix}, a linear mapping between monoarticular and biarticular exoskeletons can be established to relate these two device through a linear jacobian as it is represented in Eqn \eqref{Eqn_Mono_Bi_Jacobian}.
...
\end{aligned}
\end{equation}
Using Eqn.\eqref{Eqn_Mono_Bi_Jacobian} which is a mapping between the angular velocities of the exoskeletons, we can derive the mapping between the provided torque by exoskeletons as shown in Eqn. \eqref{Mono_Bi_Torque_Mapping}.
...
%TODO: The professor asked me to have more detailed explaination on metabolic model.
...
%TODO: This paragraph needs to be explicitly define power calculation
...
\paragraph*{Regeneration effect on devices power consumption.} 
...
%TODO: This analysis has been changed and I want to explain it more explicitly
