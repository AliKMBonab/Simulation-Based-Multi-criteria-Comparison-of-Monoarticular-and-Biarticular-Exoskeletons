...
metabolic_rate_data = np.concatenate((np.reshape(unassisted_energy_dataset['noload_metabolics_energy'],(7,3),order='F'),\
                                np.reshape(assisted_energy_dataset['monoarticular_hip60knee70_noload_metabolics_energy'],(7,3),order='F'),\
                                np.reshape(assisted_energy_dataset['biarticular_hip50knee60_noload_metabolics_energy'],(7,3),order='F')),axis=0)
final_dataset = np.column_stack([assistance_col,metabolic_rate_data])
final_dataset = np.column_stack([subject_col,final_dataset])
with open(r'.\Statistics\Specific_Weights\NoloadMono06_NoloadBi12\MetabolicRate_Dataset.csv', 'wb') as f:
...
assistive_actuators_avg_totalpower_data = np.concatenate((np.reshape(assisted_energy_dataset['biarticular_hip50knee60_noload_hipactuator_energy'],(7,3),order='F'),\
                                                        np.reshape(assisted_energy_dataset['biarticular_hip50knee60_noload_kneeactuator_energy'],(7,3),order='F'),\
                                                        np.reshape(assisted_energy_dataset['monoarticular_hip60knee70_noload_hipactuator_energy'],(7,3),order='F'),\
                                                        np.reshape(assisted_energy_dataset['monoarticular_hip60knee70_noload_kneeactuator_energy'],(7,3),order='F')),axis=0)
