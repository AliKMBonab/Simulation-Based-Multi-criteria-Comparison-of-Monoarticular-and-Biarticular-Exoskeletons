...
# muscles activation
# monoarticular
mono_noload_muscles_activation = utils.normalize_direction_data(musclesactivation_dataset['monoarticular_hip60knee70_noload_ninemuscles_activation'],gl_noload,direction=False,normalize=False)
...
mono_loaded_muscles_activation_dic = {'pgc':gait_cycle,'avg':mean_mono_loaded_muscles_activation,'label':'Noload',
                        'std':std_mono_loaded_muscles_activation,'avg_toeoff':noload_mean_toe_off}
...
utils.plot_muscles_avg(plot_dic=muscles_activation_monoarticular_noload_plot_dic,toeoff_color='xkcd:shamrock green',color=mycolors['french rose'],is_std=True)
plt.legend(loc='best',frameon=False)
plt.show()
fig.tight_layout()
fig.savefig('./Figures/Specific_Weights/NoloadMono06_LoadedMono04/Monoarticular_Noload_MusclesActivation.pdf',orientation='landscape',bbox_inches='tight')

# monoarticular
fig, ax = plt.subplots(num='Monoarticular Noload Muscles Activation',figsize=(8.4, 6.8))
utils.plot_muscles_avg(plot_dic=muscles_activation_loaded_plot_dic,toeoff_color='grey',color='k',is_std=True)
utils.plot_muscles_avg(plot_dic=muscles_activation_monoarticular_loaded_plot_dic,toeoff_color='grey',color=mycolors['crimson red'],is_std=True)
plt.legend(loc='best',frameon=False)
plt.show()
fig.tight_layout()
fig.savefig('./Figures/Specific_Weights/NoloadMono06_LoadedMono04/Monoarticular_Loaded_MusclesActivation.pdf',orientation='landscape',bbox_inches='tight')
...
plt.show()
fig.tight_layout()
fig.savefig('./Figures/Specific_Weights/NoloadMono06_LoadedMono04/Monoarticular_NoloadVsLoaded_MusclesActivation.pdf',orientation='landscape',bbox_inches='tight')
