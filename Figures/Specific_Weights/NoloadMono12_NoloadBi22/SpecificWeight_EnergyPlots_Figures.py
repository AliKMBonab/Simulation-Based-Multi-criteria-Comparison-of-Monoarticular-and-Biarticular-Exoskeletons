...
directory = './Data/Specific_Weights/*_energy.csv'
...
noload_mono_metabolics = utils.reduction_calc(unassisted_energy_dataset['noload_metabolics_energy'],assisted_energy_dataset['monoarticular_hip50knee60_noload_metabolics_energy'])
...
mean_noload_mono_energy,std_noload_mono_energy = utils.mean_std_over_subjects(assisted_energy_dataset['monoarticular_hip50knee60_noload_metabolics_energy'],ax=0)
#####################################################################################
...
metabolic_rate_data = np.concatenate((np.reshape(unassisted_energy_dataset['noload_metabolics_energy'],(7,3)),\
                                np.reshape(assisted_energy_dataset['monoarticular_hip50knee60_noload_metabolics_energy'],(7,3)),\
...
with open(r'.\Statistics\Specific_Weights\NoloadMono12_NoloadBi22\MetabolicRate_Dataset.csv', 'wb') as f:
...
assistive_actuators_avg_totalpower_data = np.concatenate((np.reshape(assisted_energy_dataset['biarticular_hip30knee50_noload_hipactuator_energy'],(7,3)),\
                                                        ...
                                                        np.reshape(assisted_energy_dataset['monoarticular_hip50knee60_noload_hipactuator_energy'],(7,3)),\
                                                        np.reshape(assisted_energy_dataset['monoarticular_hip50knee60_noload_kneeactuator_energy'],(7,3))),axis=0)
final_dataset = np.column_stack([assistive_actuators_col,assistive_actuators_avg_totalpower_data])
final_dataset = np.column_stack([subject_col,final_dataset])
with open(r'.\Statistics\Specific_Weights\NoloadMono12_NoloadBi22\ActuatorsAvgPower_Dataset.csv', 'wb') as f:
  f.write(bytes(utils.listToString(headers)+'\n','UTF-8'))
  np.savetxt(f, final_dataset, fmt='%s', delimiter=",")
#####################################################################################
# Plots
...
fig.savefig('./Figures/Specific_Weights/NoloadMono12_NoloadBi22/Metabolic_Rate_BarPlot.pdf',orientation='landscape',bbox_inches='tight')

#******************************************************************
# Metabolic Bar Plot
# noload Mono Vs Bi
names = ['bi, unloaded','mono, unloaded']
...
fig.tight_layout()
plt.show()
fig.savefig('./Figures/Specific_Weights/NoloadMono12_NoloadBi22/Metabolic_Percent_ParPlot.pdf',orientation='landscape',bbox_inches='tight')
...
names = ['unassist, unloaded','mono, unloaded','bi, unloaded']
x = np.arange(1,len(names)+1,1)
data = [utils.mean_over_trials(unassisted_energy_dataset['noload_metabolics_energy']),\
        utils.mean_over_trials(assisted_energy_dataset['monoarticular_hip50knee60_noload_metabolics_energy']),\
...
fig.tight_layout()
plt.show()
fig.savefig('./Figures/Specific_Weights/NoloadMono12_NoloadBi22/Metabolic_Rate_BoxPlot.pdf',orientation='landscape',bbox_inches='tight')


#******************************************************************
# Actuators Energy Box Plot
# Biarticular Vs Monoarticular noload
names = ['bi hip, unloaded','bi knee, unloaded','mono hip, unloaded','mono knee, unloaded',]
x = np.arange(1,len(names)+1,1)
data = [utils.mean_over_trials(assisted_energy_dataset['biarticular_hip30knee50_noload_hipactuator_energy']),\
        ...
        utils.mean_over_trials(assisted_energy_dataset['monoarticular_hip50knee60_noload_hipactuator_energy']),\
        utils.mean_over_trials(assisted_energy_dataset['monoarticular_hip50knee60_noload_kneeactuator_energy'])]
fig, ax = plt.subplots(figsize=(6.4, 4.8))
bp = ax.boxplot(data, patch_artist=True)
utils.beautiful_boxplot(bp)
...
fig.tight_layout()
plt.show()
fig.savefig('./Figures/Specific_Weights/NoloadMono12_NoloadBi22/Actuator_Energy_BoxPlot.pdf',orientation='landscape',bbox_inches='tight')
...
fig.savefig('./Figures/Specific_Weights/NoloadMono12_NoloadBi22/PaperFigure_BoxPlot.pdf',orientation='landscape',bbox_inches='tight')
plt.show()
